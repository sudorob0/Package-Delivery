        ...
        if selection == "1":
            ...
            for package_id in range(1, num_of_packages):
                ...
                if package.deadline == "EOD":
                    format = "\t"
                print(f"{package.id}\t\t| {package.deadline}{format}\t| {package.status}\t| {timestamp}\t\t| {package.address}")
        elif selection == "3":
            print("Truck #\t\t| Departure\t| Mileage\t| Location\t\t| Packages")
            print("-" * 80)
            print(f"Truck 1\t\t| {truck1.depart_time}\t| {truck1.mileage}\t\t| {truck1.address}\t| {truck1.packages}")
            print(f"Truck 2\t\t| {truck2.depart_time}\t| {truck2.mileage}\t\t| {truck2.address}\t| {truck2.packages}")
            print(f"Truck 3\t\t| {truck3.depart_time}\t| {truck3.mileage}\t\t| {truck3.address}\t| {truck3.packages}")
        elif selection == "4":
            user_package_id = input('Enter package ID:\n').strip()
            package = package_hash_table.search(int(user_package_id))
            # Update departure and delivery time if packages are on its way or delivered
            timestamp = "Departure Time: Awaiting pickup\nDelivery Time: Pending delivery"
            if "Delivered" in package.status:
